...
export async function POST(req: NextRequest) {
  try {
    ...
    const generatedReport = await generateOwnerReport({
      ...
      netIncome,
      latePayments: Number(late_payments) || 0,
      managementFee: Number(management_fee) || 0,
      vacantUnits: Number(vacant_units),
      totalUnits: totalUnitsNum,
      outstandingBalances: Number(outstanding_balances) || 0,
      upcomingExpirations: upcoming_expirations || '',
      ytdRent: Number(ytd_rent) || 0,
      ytdExpenses: Number(ytd_expenses) || 0,
